    ...
    function toRapidResponse(error) {
      try {
        if (util.types.isNativeError(error) || _isError(error)) {
          return {
            errorType: error.name?.replaceAll("", "%7F"),
            errorMessage: error.message?.replaceAll("", "%7F"),
            trace: error.stack.replaceAll("", "%7F").split("\n")
          };
        } else {
          return {
  ...
  "XRayError.js"(exports, module) {
    ...
    var XRayFormattedCause = class {
      constructor(err) {
        ...
        let stack = [];
        if (err.stack) {
          let stackLines = err.stack.replaceAll("", "%7F").split("\n");
          stackLines.shift();
        ...
        this.exceptions = [
          {
            type: err.name?.replaceAll("", "%7F"),
            message: err.message?.replaceAll("", "%7F"),
            stack
          }
        ];
  ...
  "LogPatch.js"(exports, module) {
    ...
    function formatJsonMessage(requestId, timestamp, level, ...messageParams) {
      let result = {
        timestamp,
        level: level.name,
        message: util.format(...messageParams),
        requestId
      };
    ...
    var logTextToStdout = (level, message, ...params) => {
      let time = new Date().toISOString();
      let requestId = _currentRequestId.get();
      let line = `${time}	${requestId}	${level.name}	${util.format(message, ...params)}`;
      line = line.replace(/\n/g, "\r");
      process.stdout.write(line + "\n");
    };
    var logJsonToStdout = (level, message, ...params) => {
      let time = new Date().toISOString();
      let requestId = _currentRequestId.get();
    ...
    var logTextToFd = function (logTarget) {
      ...
      return (level, message, ...params) => {
        ...
        let requestId = _currentRequestId.get();
        let enrichedMessage = `${time}	${requestId}	${level.name}	${util.format(message, ...params)}
`;
    ...
    var logJsonToFd = function (logTarget) {
      ...
      return (level, message, ...params) => {
        ...
        let time = date.toISOString();
        let requestId = _currentRequestId.get();
        let enrichedMessage = formatJsonMessage(requestId, time, level, message, ...params);
        typeAndLength.writeUInt32BE((2774138882 | level.tlvMask) >>> 0, 0);
        let messageBytes = Buffer.from(enrichedMessage, "utf8");
        typeAndLength.writeInt32BE(messageBytes.length, 4);
        typeAndLength.writeBigInt64BE(BigInt(date.valueOf()) * 1000n, 8);
        fs.writeSync(logTarget, typeAndLength);
    ...
    function _patchConsoleWith(log) {
      ...
      let awsLambdaLogLevel = levels[process.env["AWS_LAMBDA_LOG_LEVEL"]?.toUpperCase()] ?? levels.TRACE;
    ...
    var _patchConsole = () => {
      const JsonName = "JSON", TextName = "TEXT";
      let awsLambdaLogFormat = process.env["AWS_LAMBDA_LOG_FORMAT"]?.toUpperCase() === JsonName ? JsonName : TextName;
      let logger;
      if (process.env["_LAMBDA_TELEMETRY_LOG_FD"] != null && process.env["_LAMBDA_TELEMETRY_LOG_FD"] != void 0) {
        ...
        logger = awsLambdaLogFormat === JsonName ? logJsonToFd(logFd) : logTextToFd(logFd);
      } else {
        logger = awsLambdaLogFormat === JsonName ? logJsonToStdout : logTextToStdout;
      }
...
await meadowlarkLambdaRuntimeStart();
